"""Core client for interacting with the Notix API.

Enhancements:
- Optional ``raise_on_error`` to raise ``NotixHTTPError`` on non-2xx.
...
DEFAULT_BASE_URL = "https://app.usenotix.dev"


class NotixHTTPError(Exception):
...
class Notix:
    ...
    ----------
    key:
        API key issued by Notix. If not provided, the client attempts to
        read ``NOTIX_API_KEY`` from the environment.
    url:
    ...
    def __init__(
        ...
        *,
        ...
        self.key = key or os.getenv("NOTIX_API_KEY")
        if not self.key:
            raise ValueError("Missing API key. Pass it to Notix('notix_123')")

        base = os.getenv("NOTIX_BASE_URL") or DEFAULT_BASE_URL
        if url:
            base = url
    ...
    def webhooks(self, secret: str) -> "Webhooks":
        ...
        -------
        ```python
        notix = Notix("notix_12345")
        webhooks = notix.webhooks("whsec_xxx")
    ...
    def _request(
        ...
        if not resp.ok:
            ...
            if self.raise_on_error:
                raise NotixHTTPError(resp.status_code, error, method, path)
            return None, error
...
from .auth import Auth  # noqa: E402  pylint: disable=wrong-import-position
...
from .segments import Segments  # noqa: E402  pylint: disable=wrong-import-position
from .domains import Domains  # type: ignore  # noqa: E402
from .campaigns import Campaigns  # type: ignore  # noqa: E402
from .sms import Sms  # noqa: E402  pylint: disable=wrong-import-position
from .verify import Verify  # noqa: E402  pylint: disable=wrong-import-position
from .journeys import Journeys  # noqa: E402  pylint: disable=wrong-import-position
from .deliverability import Deliverability  # noqa: E402  pylint: disable=wrong-import-position
from .analytics import Analytics  # noqa: E402  pylint: disable=wrong-import-position
